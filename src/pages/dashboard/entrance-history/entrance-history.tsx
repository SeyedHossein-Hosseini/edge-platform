...
import { Alert, Autocomplete, Color, Pagination } from "@material-ui/lab";
...
const EntranceHistory = () => {
  ...
  const [exportCsvLoading, setExportCsvLoading] = React.useState(false);
  const [exportCsvSuccess, setExportCsvSuccess] = React.useState(false);

  const exportCsvButtonClass = clsx({
    [classes.exportCsvButtonSuccess]: exportCsvSuccess,
  });
  ...
  const getEntranceRecords = () => {
    ...
    const queryParams: any = {
      ...
      ...(endDate && {
        timestamp__lte: moment(endDate).add(1, "m").toISOString(),
      }),
      ...(selectedEmployee.length > 0 && {
        user_id: selectedEmployeeId.toString(),
      }),
      ...(selectedFaceCameras.length > 0 && {
        cameras: selectedFaceCameras.toString(),
      }),
    };
    detectedFacesLogsData(queryParams).subscribe({
      next: (res: any) => {
        ...
        setCount((prevState: number) => {
          const recordsLength: number =
            res.count !== undefined ? Math.ceil(res.count / perPage) : 100;
  ...
  const getCsv = () => {
    ...
    if (!exportCsvLoading) {
      ...
      getCsvLink(queryParams).subscribe({
        next: (data: any) => {
          ...
          setExportCsvColor("success");
          setExportCsvMessage("Data is successfuly downloaded");
          console.log(data);
        },
        error: (error: Error) => {
          console.log(error);
          setExportCsvLoading(false);
          setExportCsvColor("error");
          setExportCsvMessage(error.message);
        },
      });
      ...
      <div className={classes.wrapper}>
            ...
            {records.length > 0 &&
              records.map((item: any, index: number) => (
                <Grid item xs={12} lg={12} key={index}>
                  <EntranceLogCard
                    avatar={
                      "http://192.168.3.201:8000" +
                      item["log"]["employee"]["avatar"]
                    }
        ...
        <Sticky>
          <Grid container spacing={3}>
              ...
              <FormControl variant="outlined" fullWidth>
                <Autocomplete
                  ...
                  onInputChange={(_, value: string) =>
                    filterEmployeeByLastName(value)
                  }
                  filterOptions={(x) => x}
            ...
            <Grid item xs={12} lg={12}>
              <FormControl variant="outlined" fullWidth>
                <InputLabel id="selected_camera">
                  {t("personnel.cameraFilter")}
                </InputLabel>
                <Select
                  fullWidth
                ...
                >
                  {faceCameras.map((item: any, index: number) => (
                    <MenuItem value={item["id"]} key={index}>
                      <Checkbox
                        checked={selectedFaceCameras.indexOf(item["id"]) > -1}
                      />
            ...
            <Grid item xs={12} lg={12}>
              <div className={classes.exportCsvRoot}>
                <div className={classes.exportCsvWrapper}>
                  <Button
                    variant="contained"
                    color="primary"
                  ...
                  {exportCsvLoading && (
                    <CircularProgress
                      size={24}
                      className={classes.exportCsvbuttonProgress}
                    />
                  )}
                </div>
              </div>
